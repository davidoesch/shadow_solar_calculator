#!/bin/bash
# Script: calculate_shadows_loop_UTC.sh
# Loop version with CORRECT UTC time handling
# CORRECTLY CONFIGURED FOR: UTC time (Sentinel-2: 10:00-11:00 UTC)
# 
# IMPORTANT: This script uses civil_time=0 for proper UTC interpretation
# 
# Usage: ./calculate_shadows_loop_UTC.sh [day_of_year]
# Example: ./calculate_shadows_loop_UTC.sh 153
...
log_message "========================================"
log_message "Shadow Calculation - UTC Mode (CORRECTED LOOP)"
log_message "========================================"
...
CURRENT_TIME=$UTC_START_HOUR
STEP_COUNT=0

log_message "Starting time loop with civil_time=0 (UTC)..."
echo ""

while (( $(echo "$CURRENT_TIME < $UTC_END_HOUR" | bc -l) )); do
    ...
    # Run r.sun with civil_time=0 for UTC interpretation
    ...
    # Convert solar incidence angle to 8-bit
    grass "$GRASSDATA/$LOCATION/$MAPSET" --exec r.mapcalc \
